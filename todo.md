...
Every operation in the canonical keymap that has an implementation: Copy / Move / Delete / Rename / View / Edit / Make-new / `/`-incremental-search / Left-on-root ascend / F9 menu bar. Tree-pane Left/Right drill-in/out wired. Ctrl+F find-across-tree + Ctrl+G next-match wired. `L` log new source wired. `Ctrl+R` refresh source wired. StatusLine.flash + pane auto-refresh (both panes) as cross-cutting UX. **405/405 tests green** (was 294 at original v0 completion; +24 from tagging polish 2026-05-22; +13 from F1 Help; +11 from tree-pane tagged-node styling; +13 from tree-pane auto-refresh; +8 from tree-pane Left/Right; +14 from Ctrl+F + Ctrl+G; +16 from `L`; +12 from `Ctrl+R`, 2026-05-23). See per-era follow-up sections below for v0.x polish work; see `design.md` Decision log for the full chronological design trail.
...
- [ ] **Recursive-tag progress feedback.** The walker is `@work` so the UI stays responsive, but for very large subtrees (10k+ entries) the user gets one flash at the end and no in-flight count. A periodic flash ("Tagged 1024 so far...") every N=500 entries would feel snappier; trivial to add inside the `async for` loop.
- [ ] **Recursive-tag cancellation.** Pressing Space again during an in-flight walk currently posts a second `TagRequested`. With Textual's worker exclusivity this might serialise correctly, but worth a spot-check. Eventually the user should be able to Esc the walk.
...
- [ ] **Dropdown positioned under its menu name.** v0 fixes the dropdown at top-left; MC positions each dropdown directly under the menu name in the bar (so File's dropdown is at the File column, Commands' is at the Commands column). Compute the x-offset based on the menu name positions and render the dropdown there.
- [ ] **Top-level letter accelerator from outside the menu.** Pressing `Alt+F` (or `Alt+C`) anywhere should open the File / Commands menu directly. Today only F9 enters menu mode; from there letters work. The Alt-anywhere shortcut requires top-level bindings that fire even when a pane has focus, and need to coexist with potential clashes (e.g. Alt+C is unlikely but worth checking the cross-platform alt-handling rules in design.md).
- [ ] **Mouse support for the menu bar.** Click on a top-level name to open its dropdown; click on an item to activate. Textual supports click events on widgets; integration with the modal screen would need a focus-handoff dance.
...
- [ ] **Cursor wrap from last item Down -> first item.** Today Down on the last item stays there (the wrap logic only kicks in when we walk past a separator). Should wrap to item 0 (skipping any leading separators).
...
- [ ] **Persistent menu bar focus state.** Today F9 always opens at File; after F9 + Right (-> Commands) + Esc, next F9 still opens File. Some users want "last menu wins". Trivial change once you decide it's what you want.
