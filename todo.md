...
## v0 — COMPLETE 2026-05-22

Every operation in the canonical keymap that has an implementation: Copy / Move / Delete / Rename / View / Edit / Make-new / `/`-incremental-search / Left-on-root ascend / F9 menu bar. StatusLine.flash + pane auto-refresh as cross-cutting UX. **294/294 tests green.** The only unbound F-key on the bar is F1 (Help). See per-era follow-up sections below for v0.x polish work; see `design.md` Decision log for the full chronological design trail.
...
- [ ] **Help menu + About modal.** Add a third top-level menu with at minimum an About item that opens a tiny ModalScreen showing the WTree version, attribution, and a one-line description. F1 (currently a no-op) should also open the same About modal (or a Help screen with the keymap).
