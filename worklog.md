...
## 2026-06-07 (cont.) — App-level Ctrl+D (logged-tree drive switching)

Closed the follow-up minutes after parking it: `WTreeApp.action_switch_drive` (`@work`) pushes the same `DriveChooserScreen` the picker uses (anchors from `wtree._drives`, current root always listed, initial cursor on it) and re-roots the logged tree through the `L` machinery — `_run_scan_with_dialog` gate, tags survive (absolute paths), `Logged: <new> (switched from <old>)` flash. Esc flashes cancelled; same-root pick is a silent no-op; an anchor that vanished between enumeration and pick (unplugged removable) flashes "not available" without re-rooting (the `os.path.isdir` pre-check `L` also does). No per-location cursor memory at app level — consistent with `L`, which lands fresh; the picker's memory stays a picker thing.

Surfaces: `ctrl+d` BINDING (checked free first), Commands menu "Switch drive" (accelerator `d`, right after Log new source — conceptual pair), Help Navigation row. test_menu.py shape assertions bumped (Commands 7 → 8 items; Untag-all walk 6 → 7 downs).

10 new tests in `tests/test_app_switch_drive.py` (static surface incl. menu-position assertion; pilot: pick-reroots+flash, tags-survive, Esc-cancels, same-root no-op, vanished-anchor nudge, F9 menu route). **718 → 728/728 green** (eighths). design.md drive-switching paragraph updated + decision row; todo follow-up checked off. Clean mount protocol throughout. NOT committed — Matthew commits Windows-side.

## 2026-06-07 (cont.) — Picker type-to-filter + files-greyed (phase 2 CLOSED)

Design pass (4 forks, all recommendations accepted): **jump + dim-non-matches** over pure-jump and over fzf-hide (Textual Tree has no node-hide — narrowing would mean remove/re-add churn + expansion bookkeeping); **`/` activation** over bare type-ahead (keeps `n` = new folder bindable); **files-greyed = `f` toggle default-off**; **visible-rows-only** match scope (pane-`/` consistency, no surprise auto-expansion).

Code: **`search_bar.compute_matches(rows, query, anchor)`** extracted (needle + first-at-or-after-anchor walk) — `WTreeApp.on_search_bar_query_changed` refactored onto it, picker uses the same helper (populate_dir_node drift-prevention precedent). **`populate_dir_node` grew `include_files=False`**: non-dir entries appended after the dirs as dim `Text(style="dim")` leaves with `data=None` — same convention as error placeholders, so Enter-to-pick, cursor memory, and search walks skip them with zero extra checks; TreePane call sites untouched. **`_PickerTree`**: SearchTarget protocol (`iter_searchable` counts every visible row for line numbering but yields only selectable dir rows; **root row deliberately excluded** — its label is the full anchor path and substring-matches almost any query, found via first red test run; TreePane's pane-search includes its root, the picker diverges since it has no ascend), `set_filter_matches(lines)` translating to node ids + `render_label` dim consult (tagged-styling pattern), `set_show_files` rebuild (snapshot expanded paths + cursor → re_root → re-expand shallowest-first → reveal cursor). **`DirPickerScreen`**: composes its own `SearchBar` (modal = separate DOM), `slash`/`f` bindings, the five `on_search_bar_*` handlers all `event.stop()`ed (they'd bubble past the screen to the app's handlers otherwise), `_exit_filter(restore=)` mirrors `_exit_search` (Esc restores pre-search cursor, Enter commits in place), footer hint updated.

13 new tests in `tests/test_picker_filter.py` (compute_matches units; include_files populate shape; slash-activates+jumps, dim-state + render_label spans, Esc-restores+clears+keeps-picker-open, commit-leaves-cursor+second-Enter-picks, Down-cycles-with-wrap, greyed-files-never-match; f-toggle rows+ordering, file-row-not-selectable, expansion+cursor-survive-toggle). **728 → 741/741 green** (eighths, zero flakes). design.md: new subsection paragraph + root-exclusion note + decision row; **picker phase 2 closed**. Clean mount protocol. NOT committed — Matthew commits Windows-side.

## 2026-06-07 (cont.) — Cursor preservation on refresh (both panes)

Took the daily-friction item: delete row 5, cursor dumped to row 0. Two forks settled with Matthew (both = recommendations): **same-row-index clamped** fallback when the entry is gone (Explorer/MC convention — the cursor lands on what is now row N, i.e. the next sibling, for free on single deletes); **both panes** this pass (the tree side's own parked follow-up included).

**Contents pane:** `show_path` detects *refresh vs navigation* with no new kwarg — re-showing the **same** path (auto-refresh after ops, Ctrl+R, editor return) preserves; a **different** path is navigation and still pins row 0. Snapshot before the scan (cursor row + backing path from `_row_paths`); the commit block restores by entry if it survives (stable across insertions above it), else clamps the old index to the new length. The scan-cancel path never reaches the commit block, so a cancelled refresh still leaves everything untouched.

**Tree pane:** `refresh_paths` snapshots `cursor_node.data` before the rebuild and `reveal_path`-restores after (gated on `matches` — untouched trees don't move the cursor; one `asyncio.sleep(0)` first for the line-indexer trap). Deleted entry → walk `os.path.dirname` upward to the **nearest surviving ancestor** — the tree analogue of the flat-row clamp (a line index has no stable meaning after subtree wipes; the parent dir is where the deleted entry *was*). The old "best-effort, wherever Textual puts it" docstring paragraph replaced with the real contract.

7 new tests in `tests/test_cursor_preserve.py` (entry-survives-insertion-above, delete-mid clamp → next sibling, delete-tail clamp → new last, navigation-still-resets, tree cursor survives refresh, tree fallback-to-ancestor, **e2e: D + confirm on row N → auto-refresh → cursor on new row N** — the exact scenario the item came from). All pre-existing refresh/scan/app suites untouched and green. **741 → 748/748** (eighths, zero flakes). design.md decision row added. Clean mount protocol. NOT committed — Matthew commits Windows-side.

## 2026-06-07 (cont.) — Overlapping-tag dedup (parked semantics call CLOSED)

Grounding pass revealed this was a **correctness bug**, not just the parked perf lever: the destination mapping is `dest/basename(tag)`, so recursive-tag + Copy produced **flattened duplicate siblings** (`dest/foo` + `dest/bar` + `dest/baz.txt` for a fully-tagged `foo/bar/baz.txt`); Move emitted items whose source had already moved inside an earlier item; Delete emitted already-gone failure noise. Two forks settled with Matthew (both = recommendations): **all three per-tag planners** (Rename single-entry exempt); **flash the collapse count** at enqueue so a 349k-tag plan showing 1 item is explained.

`ops/base.collapse_nested_tags(tags, *, case_insensitive=None)`: containment on `canonical_path` (separators + platform case rule, flag parameterised for sandbox testing), same-source only, processed in segment-depth order with each tag's dirname chain checked against the kept set — **immune to the lexicographic sibling trap** (`/a-x` sorts between `/a` and `/a/b`; a naive last-kept-prefix scan would either drop `/a-x` or keep `/a/b`). Canonical duplicates (mixed separators) collapse too. Tagged set never touched — plan-time transform only. `Plan` grew `collapsed_tags: int = 0` (non-frozen dataclass, survives the conflict passes which never reconstruct it); planners thread the count; the common enqueue tail flashes when > 0. Properties tagged-mode double-count noted as a parked display concern.

11 new tests in `tests/test_tag_dedup.py`: 7 helper units (topmost-collapse, the sibling trap, disjoint order preservation, source isolation, mixed separators, both case rules, canonical-duplicate), 3 planner integrations (copy/move/delete `collapsed_tags` + item shapes), and the **e2e proof: recursive-tag foo → C → dest contains ONLY `foo/`** (was foo + bar + baz.txt before). One first-cut bug caught by my own lint gate pattern: unused `OperationQueue` import + `status` local in the new test file — swept before they could land. **748 → 759/759 green** (eighths, zero flakes). design.md: new "Overlapping-tag collapse" subsection + decision row. Clean mount protocol. NOT committed — Matthew commits Windows-side.

## 2026-06-07 (cont.) — Crash-handler phase 2 (re-vendor + owned exit screen + thread nets)

Matthew dropped an updated upstream into the repo root (`Python Errorhandler New`, 2026-06-06 build — 2702 lines vs the 1384 vendored at `645aae9`; install/uninstall/install_asyncio hooks, @capture/capturing, ReportFormatter, observers, to_markdown, PEP 657 anchors, frame origin tags + skip_modules, max_report_bytes budget, more extractors) and delegated integration choices. **Verified the drop's own suite first: 147 tests OK (11 platform skips).** Re-vendored verbatim under an updated provenance header (date-based — the drop has no git rev). All 11 phase-1 crash tests passed unchanged against the new file before any glue work — clean upstream compat.

**Wiring (as-I-saw-fit, per delegation):** `crash.build_report` policies — default = `skip_modules=("textual","rich")` (deep framework stacks collapse to honest `[N hidden]` markers) + `max_report_bytes=512 KiB` (vendored budget drops locals first, then source context); `WTREE_DEBUG=1` = full fidelity (locals, no collapse, no budget). New `crash.install_thread_hooks()` from `main()` — vendored `install(hooks=("threading","unraisable"), style="concise")`; stray worker threads + `__del__` unraisables now report to stderr; **sys.excepthook slot deliberately untouched** (outer-net territory; Textual loop crashes stay with `_handle_exception`). **Deliberately unwired:** observers (our log writer would double-fire), @capture (no call sites need it), ReportFormatter (no logging subsystem), to_markdown (noted for a future GitHub-issue flow), install_asyncio (Textual owns the loop).

**Owned exit screen (the parked phase-2 option):** `WTreeApp._fatal_error` override — the seam Textual's `_handle_exception` calls for non-`__rich__` errors. Replaces the raw Rich traceback-with-locals with one clean red panel (`Type: message`, `Full report: <path>`, WTREE_DEBUG hint). Under `WTREE_DEBUG=1` the Textual-style traceback rides along **inside the same `Group`** — `_print_error_renderables` only shows `_exit_renderables[0]` outside Textual's debug feature-set, so both must share one entry (would-be-lost-renderable trap). Any failure in the override falls back to `super()._fatal_error()` — owning the screen must never mask the crash.

8 new tests in `tests/test_crash_phase2.py` (build_report policy kwargs both postures via describe_error stub + one real round-trip; install_thread_hooks wires threading + leaves sys + idempotent; stray-thread crash prints report (capsys); _fatal_error panel-not-raw-dump; debug appends traceback; phase-1 stash+log contract intact). Two test gotchas: the pilot re-raises a stashed `_exception` at run_test exit (clear the probe before leaving the ctx), and **pytest's threadexception plugin re-wraps `threading.excepthook` per test** so cross-batch identity asserts on the restored hook are unstable (dropped; upstream covers the restore contract). **759 → 767/767 green** (eighths). design.md: phase-2 paragraph + decision row. The drop folder stays untracked in the repo root — Matthew's call to keep/remove now it's vendored. NOT committed — Matthew commits Windows-side.

## 2026-06-07 (cont.) — Chooser polish (labels, free space, ~) — drive-switching era CLOSED

The last Drive-switching-era follow-up. `_drives.anchor_details(anchor)` -> best-effort `(label, free, total)`: Windows volume label via ctypes `GetVolumeInformationW` (no pywin32; pragma no cover on POSIX), `shutil.disk_usage` for the sizes, every field degrading to None on failure (unmounted letter, permission wall, dead share) — bare row beats a crash. `_drives.friendly_anchor_name` folds the home anchor to `~` for display ("/" home deliberately NOT folded — containers); the chooser still returns the real path. `DriveChooserScreen`: `_details` dict + `on_mount` decoration pass — one `asyncio.to_thread(anchor_details, ...)` per anchor, repaint per arrival, so the list paints immediately and **a dead UNC share's multi-second stat delays only its own row's details, never the modal**. Row format: `> name  [Label]  X free of Y` (padded name column, `ops.base._human_bytes` reused).

7 new tests in `tests/test_chooser_polish.py` (~ fold + root-home guard, real-mount usage sanity, bad-path degrade, decorated-row rendering with fake details, ~-display-real-path-return round-trip, raising-decorator survival). **767 → 774/774 green** (eighths, zero flakes). design.md polish sentence + decision row; drive-switching era fully closed in todo.md. NOT committed — Matthew commits Windows-side.

## 2026-06-07 (cont.) — Severity-styled flash

The flash+auto-refresh-era follow-up, exactly per its todo spec plus one safety upgrade. `StatusLine.flash` (and the `App.flash` wrapper) gained `severity=` — `info` (default) plain, `warning` yellow, `error` bold red, unknown severities render as info rather than raising. **Safety upgrade:** the message body is `rich.markup.escape`d BEFORE the severity wrapper is applied — the Static was already `markup=True`, so a user path containing `[i]` could previously style (or MarkupError) the status line; flash messages are now always literal. `_flash_severity` stored alongside `_flash_message` for tests/inspection.

Call-site classification (~35 sites): **error (9)** = real failures — planner err.message (Rename/Make-new), Edit editor-not-found / exception / non-zero exit, Log path-doesn't-exist / not-a-directory, Switch-drive not-available, Menu unknown-action. **warning (26)** = rejections + nothing-to-do nudges — nothing-under-cursor, cannot-view/edit-kind, that's-a-directory, Rename-works-on-one-entry, nothing-to-tag / already-tagged / pattern-no-matches, planner-produced-no-items (×3), all-conflicts-skipped, nothing-to-verb (×2), already-there self-target, already-at-root, focus-a-pane-first, Find no-matches / no-active-search, Properties nothing, No-operation-in-progress. **info (rest, zero edits)** = cancellations, Logged:, Tagged N, Source refreshed, Find match readouts, collapsed-tags note.

8 new tests in `tests/test_flash_severity.py` (styled-wrapper units incl. unknown-severity + markup-escape, warning/error/info integration through real key flows). Existing flash tests untouched and green (`_flash_message` still stores the raw text). **774 → 782/782 green** (zero flakes). NOT committed — Matthew commits Windows-side.

## 2026-06-07 (cont.) — Menu polish (5 of 7 era items closed) + mouse menu tests

Matthew asked for the menu polish items + at least one mouse-interaction unit test, delegating the test approach ("shortest correct path"). **Shortest correct path = `pilot.click(widget, offset=...)`** — Textual's pilot synthesises genuine Click events through the real mouse pipeline; no raw pointer/setpointer plumbing needed, and no separate todo job required.

**Landed:** (1) **Dropdown under its menu name** — new `menu_bar.menu_name_spans()` / `menu_index_at(x)` (single source of truth for the `"  Name  Name  "` row layout, shared by the dropdown offset and both click hit-tests); `_sync_children` sets panel margin-left = span_start − 2 (border+padding), File lands at the old fixed position by construction. (2) **Wrap** — found ALREADY fixed (todo item stale: `_move_cursor` wraps via `% n` unconditionally); pinned Down-on-last→first and Up-on-first→last so it can't regress. (3) **Last-menu-wins** — `app._menu_memory`, read back from the MenuScreen *instance* after `push_screen_wait` returns (the instance outlives dismissal — no return-type change needed); F9 reopens there; Alt jumps override per-opening. (4) **Alt+F/C/H** — `action_open_menu(index)` bindings. (5) **Mouse** — passive-bar click posts `MenuBar.MenuRequested(idx)` → app opens that menu; modal top-row click switches `active_menu`; dropdown click maps widget-y → item via live gutter math (`content_region.y - region.y`, not hardcoded CSS) and activates; separator clicks no-op (menu stays open).

**Gotcha worth its pin:** `push_screen_wait` requires a worker context, and **parameterised binding actions (`"open_menu(1)"`) are dispatched WITHOUT one** — plain `action_menu_bar` had been getting a worker by dispatch-path luck. Both menu actions are now uniformly `@work`; the MenuRequested handler delegates to the @work action rather than `asyncio.create_task` (a bare task is not a worker). Also self-inflicted en route: a doubled `@work` decorator → `WorkerDeclarationError` at import (outer decorator sees the inner's non-async wrapper).
...
## 2026-06-10 — In-viewer `/` search (View-era follow-up)

Took the most-used pager feature, design-first per the usual rhythm (theme -> slice -> one real fork, all recommendations accepted): **slice = in-viewer search first** (self-contained, no dependency on the parked rendering rework); **post-commit fork = persistent pager-style `n`/`N`** over a minimal search-ends-on-Enter.

The viewer is a `ModalScreen`, so it follows the destination picker's precedent: it composes its own `SearchBar` (the same widget the panes use), and the five `on_search_bar_*` handlers are each `event.stop()`ed so they never bubble to the app's pane-search handlers on the screen behind the modal. The pager's twist over the row-cursor surfaces is that it searches **lines of text**, so a match is a `(line, start, end)` span, not a row index — new `viewer.find_matches` returns every occurrence (a line can hold several) with **absolute** char offsets for `Text.stylize`, via `re.finditer(re.escape(query), text, re.IGNORECASE)` (matches against the original string, so offsets stay valid even where `str.lower()` would change length and desync them — `compute_matches`' row model carries none of this, hence a viewer-local helper kept to the same substring-CI rule in spirit). Line numbers counted incrementally with `str.count("\n", prev, i)` (C-level; cheap on big files).

Behaviour: typing jumps to the first match **at or after the line on screen when `/` was pressed** (wraps; mirrors the pane anchor — captured as `int(scroll.scroll_y)`); every match styled, current one distinct (`_CURRENT_MATCH_STYLE` cyan vs `_MATCH_STYLE` yellow — provisional palette, theme pass parked). Down/Ctrl+G/Up step (wrap) while the bar is open. **Enter commits**: bar closes, highlights + position stay, `_committed` flips and screen-level `n`/`N` go live (no-op before commit). **Two-stage Esc**: bar-open Esc restores the pre-search scroll; committed first-Esc clears (back to plain view), second Esc — or `q` at any time — dismisses (Esc two-stage mirrors the Properties dialog's first-Esc-cancels precedent; `q` is the unambiguous always-quit). `/` is a no-op on a refusal body (binary / too-large / unreadable) via the `_searchable` guard. The hint line flips to `n / N next / prev   Esc clear   Q close` while a committed search is live.

One correctness drive-by: the successful-load body now renders through the same highlighter as a literal Rich `Text` (was `Static.update(str)` — a file containing `[bold]`-style text would have been parsed as console markup). Single render path; refusal bodies unchanged. `escape`/`q` were split: `escape` -> new `action_escape` (two-stage), `q` -> `action_dismiss_screen` (always dismiss); the existing escape/q-dismiss tests still pass (no committed search -> escape dismisses).

22 new tests in `tests/test_viewer_search.py` (8 `find_matches` units incl. multi-per-line, CI, non-overlap, literal-regex, Unicode-offset-validity; pilot: activation+hint-hide, highlight+count, at-or-after anchor, Down/Up/Ctrl+G wrap, Enter-keeps-highlights, `n`/`N` after commit, `n`-noop-before-commit, Esc-cancels-open, two-stage-Esc, `q`-quits-committed, no-match-red, `/`-noop-on-binary, scroll-into-view). Span assertions check positions + current-is-distinct rather than string-comparing the resolved Textual `Style`. **799 -> 821/821 green** (viewer / search / picker / menu / help / keybar slices + the pyflakes lint gate, all clean). Same clean-mount protocol (sandbox author -> md5-verified atomic push-back -> slice re-run against the mount). NOT committed — Matthew commits Windows-side.

## 2026-06-11 — Operation log (~/.wtree/operations.log) + the "copy copies nothing" field report

Matthew reported Copy "not copying files" — folders appeared at the destination but no files. Diagnosis walk: planner -> conflicts -> queue -> executor all read clean, and the copy/execute/conflict slices (49 tests) plus a fresh headless plan_copy+apply_plan repro were green in the sandbox. The decisive detail came from Matthew: **he stopped the copy partway through, and a "done with errors" toast flashed**. That is cancel-by-design behaviour — Esc on the progress dialog cancels the operation: the in-flight file's partial dst is deleted, every not-yet-started item is SKIPPED("cancelled"), and already-mkdir'd directories stay — which reads exactly as "it made folders but copied no files" when the cancel lands early. Files completed BEFORE the cancel should survive; if a future run shows they don't, that's a real bug and now we'd see it, because:

**The structural gap the report exposed: the toast was the only surface naming per-item failures, and it fades.** New `wtree/oplog.py` — `write_result` appends every completed plan to `~/.wtree/operations.log`: header line (UTC stamp + the same `summary()` the toast shows), detail line per non-SUCCESS item (`STATUS  src -> dst: message`; Delete's sentinel dst collapses the arrow), quiet-on-success so a clean 10k-file copy is one line. Crash-log posture throughout: never raises (returns None on any I/O failure), never written inside the flaky project mount, 1 MiB one-generation rotation via pre-write `os.replace` (atomic on Windows). Wired in `_on_plan_complete`; the done-with-errors toast now appends `Details: <path>`. Parked: verbose (log SUCCESS too) toggle, in-app last-operation viewer.

11 new tests in `tests/test_oplog.py` (5 format units incl. delete-arrow collapse + empty-message placeholder; 4 write units incl. chronological append, parent-dir creation, rotation-with-.1-overwrite, never-raise via parent-is-a-file; 2 pilot e2e: success writes one quiet line, forced executor failure (patched `_chunked_copy`) writes FAILED detail AND the toast names the log path). **821 -> 831 passed + 1 skipped, all slices green.**

**Mount-rules war story (the rules earned their keep):** an in-place `Edit` of `app.py` on the mount silently truncated the file tail mid-token AND left the old mtime, so Python kept loading the stale `.pyc` — tests exercised pre-edit bytecode while greps showed post-edit source. Recovery: rebuild from `git show HEAD:wtree/app.py` + re-apply the two edits in the sandbox, `ast.parse` gate, md5-verified atomic push-back, `__pycache__` purge + `touch`. Lesson pinned in todo: NEVER in-place-edit mount source files; always stage-and-replace, and purge `__pycache__` after any push-back (the mount's frozen mtimes defeat the import cache's staleness check). NOT committed — Matthew commits Windows-side.

## 2026-06-11 (cont.) — The op log catches its first bug same-day: backslash tag paths broke Copy/Move/Rename on Windows

Matthew re-ran the failing copy with the fresh op log in place: **`copy done: 0 ok 105 failed`**, every line `OSError: [WinError 123] ... 'C:\\Users\\Admin\\Desktop\\2\\C:'`. The arrow told the whole story: `dst = C:/Users/Admin/Desktop/2/` + **the entire backslashed source path**. Root cause: Windows tree-pane tag paths are native-backslashed; the planners' path math is `posixpath`-only, so `_basename("C:\\a\\b")` returns the WHOLE string and the dst mapping `dest/basename(tag)` degenerates to `dest/<full source path>`. Windows refuses the mid-path `C:` (WinError 123) — but `os.makedirs(parent)` creates ancestors before failing, so the typed destination folder appears while zero files land. Exactly Matthew's original "copy copies nothing but makes the folder" report; the cancel-semantics theory was only ever a partial account (his Ctrl+Break kills muddied the first runs, but even a clean run would have failed all items). The 2026-06-04 normalisation pass covered typed destinations; tag paths slipped through, and the Linux-only suite can never produce a backslash path from a real tree.

**Fix (planner-level, defensive):** plan_copy `to_posix`es the tag + walked paths before `_basename`/`_relative_under`; plan_move likewise for its single dst join; plan_rename runs `_basename`/`_parent` on the to_posix'd path (pre-fix `posixpath.dirname` returned `""` and the rename target degraded to a bare CWD-relative name — silently renaming into the process CWD; never field-reported, caught by inspection). `_basename` in copy/move also maps a bare drive anchor (`C:` after strip) to `""` so a tagged drive root goes down the existing unrooted path instead of building `dest/C:`. Sources stay exactly as tagged — OS calls accept native separators; only dst construction normalises. Parked thought (todo): a boundary-layer fix (panes born POSIX-flavoured) would also clean mixed-separator display and Windows `touched_paths` refresh matching.

8 new tests in `tests/test_windows_paths.py` — pure string-level pins runnable on POSIX: clean-POSIX dst sets for copy (incl. mixed-separator walk keys, exactly what `_walk_from` produces), move, rename-keeps-parent, rename-NoChange-detection, drive-root skip (copy) / UnrootedTag (move), `_basename` guard units. Affected slices re-run green: **329 passed + 1 skip** across windows-paths / ops-copy / ops-move / ops-rename / tag-dedup / copy-e2e / move-e2e / conflicts / self-target / path-norm / oplog / lint / ops-execute / delete+make-new e2e / midplan-cancel / plan-cancellable / rename-smart-cursor. Mount protocol followed throughout (HEAD-baseline stage, anchor-asserting patches, ast gate, md5-verified atomic push, `__pycache__` purge + touch). NOT committed — Matthew commits Windows-side.

## 2026-06-11 (cont.) — Selection rule follows the focused pane

Matthew's third catch of the day, mid-victory-lap on the backslash fix (the copy landed — `2\\-=GubMint Email=-` fully populated): tree cursor highlighted on the new folder, D pressed — the confirm dialog named `...\\Old 06-07-2022`, the first ROW of the folder's listing. `_resolve_selection_tags` always read the contents pane's cursor; with the tree focused, the coupled contents pane shows the highlighted dir's children with its row cursor on row 0, so every tree-focused op acted on an entry the user never picked. `action_properties` had already documented focused-pane-cursor as "the existing op convention (View / Edit / Rename)" — the ops resolver was the outlier, and every older op test pressed Tab before the op key, which is exactly why it never surfaced.

Fix: `_resolve_selection_tags` branches on `self.focused` — TreePane focused -> cursor node's dir path (error-placeholder `data is None` resolves to nothing, flashing "nothing to <verb>" instead of silently using the other pane); otherwise the contents row cursor as before; tagged set still always wins. 5 new tests in `tests/test_selection_focus.py` (delete-from-tree names the dir not its first child; copy-from-tree prompt title; tagged-set-wins with tree focused; Tab'd contents behaviour unchanged; direct resolver unit). Affected slices re-run green (239 passed + 1 skip this sweep): **suite 839 -> 844 passed + 1 skip.** One test-authoring stumble: `TaggedSet.add(source_id, path)` takes two positionals, not a `Tag`. Parked follow-up in todo: Rename from the tree pane (its action reads the contents pane directly; single-entry basename op deserves its own design look). Mount protocol followed (stage, ast gate, md5 push-back, `__pycache__` purge + touch). NOT committed — Matthew commits Windows-side.

## 2026-06-11 (cont.) — Op log catch #2: read-only git objects break Delete on Windows (+ per-entry log cap)

Matthew deleted his test copy of `-=Programming=-` (which contains git repos): `delete done: 0 ok 1 failed`, four attempts, every one `PermissionError: [WinError 5] Access is denied: '...\\.git\\objects\\03\\0f270...'`. Git marks object/pack files read-only by design; Windows refuses to unlink read-only files; bare `shutil.rmtree` stops at the first refusal. POSIX puts deletion rights on the parent directory instead, so the Linux-run suite was structurally blind to it — same blindness class as the backslash bug, now twice in one day.

Fix: `_clear_readonly_and_retry(func, path, exc)` in `execute.py` — PermissionError ONLY; ORs the owner-write bit onto the failing path AND its parent (the parent chmod covers the POSIX write-protected-dir analogue, harmless on Windows); retries the failed call once; re-raises the ORIGINAL error if the retry fails too (the true message must surface in the FAILED item, not the retry's). Wrapped as `_rmtree_force` (3.12+ `onexc` / pre-3.12 `onerror` picked per runtime — `onerror` is slated for removal) and `_unlink_force`; wired into `_native_delete` (DIR + FILE/SYMLINK branches) and the OVERWRITE pre-step `_remove_existing_blocking` (all three branches). Deliberately NOT touched: cross-fs DIR move's internal `shutil.move` rmtree — same exposure, parked in todo with the walked-progress dir-move item.

Also closed the log-bloat hole spotted when Matthew's mass-failure log hit tens of MB: rotation bounds the file BETWEEN writes, not one write, so a 100k-item failed plan would append one monstrous entry. `MAX_DETAIL_LINES = 200` per operation + honest `... and N more non-success item(s)` tail line.

10 new tests: `tests/test_delete_readonly.py` (8 — POSIX-analogue write-protected-dir staging incl. a bare-rmtree-refuses sanity pin proving the fixture stages the bug, `_native_delete` end-to-end SUCCESS, overwrite pre-step, `_unlink_force` both shapes, handler re-raise rules ×2; permission tests skip under euid 0; finally-blocks chmod the fixture back so a red test can't strand an undeletable tmp_path) + 2 cap tests in `test_oplog.py`. Affected slices green: **844 -> 854 passed + 1 skip.** Mount protocol followed. NOT committed — Matthew commits Windows-side; one stray on his side: the earlier mass-failure `operations.log` (he mentioned ~85 MB — likely the rotated `.1`) can be deleted freely, it's just a log.

## 2026-06-11 (close) — Session handed to the field run

Batch of four (op log; backslash dst fix; focused-pane Selection rule; read-only-delete fix + log cap) complete at **854 passed + 1 skip**, all staged on disk, docs current. Commit message handed to Matthew (single commit, Claude co-author tag) - he commits Windows-side, then field-runs the real workflows; `~/.wtree/operations.log` self-documents any new failure. Next pickups listed in todo (field-run triage first; then op-log follow-ups / cross-fs-move read-only exposure / rename-from-tree / boundary path normalisation / View-era cluster). Matthew is also amending the global working agreement: when the sandbox can't delete a file (FUSE unlink walls) or it's simply faster done by hand, palm the job off to him explicitly - he'd rather be handed a deletion list than have sessions fight the mount.

## 2026-06-14 — Progress dialog: taller box + ETA (remaining-time) readout

Field run is live and healthy (Matthew screenshotted a real copy mid-flight: 6115/357241 files, 24.2 MB/s, op log behaving) — so the 2026-06-11 batch landed clean as commit `a02f1cb`. Two UI asks this session, both on `ProgressScreen`.

1. **Box +3 rows:** `DEFAULT_CSS` `ProgressScreen > Vertical` height `13 -> 16`. No test pinned it.

2. **ETA field:** new seventh readout, paired into the (previously empty) right column of the Files row via the existing `_row` helper — `Files  N / M    ETA  MM:SS`. Pure helper `_eta_seconds(elapsed, bytes_done, bytes_total)` (tested without instantiating the screen, same posture as `_render_bar`/`_format_*`):
   - **Projection** (Matthew's formula): `elapsed / fraction_done * fraction_left`. He specified it in percent (`elapsed / pct_complete * pct_remaining`); the factors of 100 cancel, so the code uses the byte *fraction* — identical result, smoother than the integer percent at low %.
   - **Gating** (his spec): hold an em-dash until the sample is trustworthy — release when `elapsed >= 10s` OR (`elapsed > 1s` AND `bytes_done > 1 MiB`). Constants `_ETA_MIN_ELAPSED=10.0`, `_ETA_EARLY_ELAPSED=1.0`, `_ETA_EARLY_BYTES=1 MiB`.
   - **Maths guards:** `bytes_total<=0` / `bytes_done<=0` / `elapsed<=0` / `fraction<=0` all return `None` (no div0), so the 10s branch can't divide by a zero fraction when nothing has flowed yet.
   - 8 new tests in `test_progress_dialog.py` (5 helper units: before-gate None, early-gate-needs-both, early-gate-releases=8.0s, 10s-gate-releases-below-1MiB=40.0s, halfway-symmetry remaining==elapsed, zero-input guards ×3; 2 body-level: ETA renders MM:SS once gated / em-dash before gate). **27 -> 35 in that file.**

**Drive-by:** lint gate was red in-sandbox — `test_delete_readonly.py` (last session) had unused `OperationKind`/`Resolution` imports. Removed them; pyflakes clean across `wtree/` + `tests/` (error_handler.py stays vendored-excluded). Likely the 2026-06-11 delete-readonly session didn't re-run the lint slice after adding that test.

Verified: lint + both progress slices + delete-readonly = **67 passed**. Mount protocol followed throughout (sandbox stage, ast gate, md5-verified atomic in-dir `os.replace`, `__pycache__` purge + touch). NOT committed — Matthew commits Windows-side. Layout note handed back: the 3 extra rows are deliberate empty headroom below the readouts (body docks top, hint docks bottom); ETA sits inline on the Files row rather than spread into that space — easy to move if he'd rather it be a standalone line.

### 2026-06-14 (cont.) — Progress box width + Files/ETA column gap

Field screenshot showed `11788 / 352252ETA` — the Files count (14 chars) exactly filled `_row`'s `{value_l:<14}` field, so the right column's ETA label abutted it. Matthew asked to widen the box; the real cause was the column padding (box width can't insert an inter-column gap). Did both: `ProgressScreen > Vertical` width `60 -> 66` (also gives the wrapping source-path line more room), and `_row` value_l column `14 -> 18` so the long Files count always clears the ETA label. Render-checked at 11788/352252 → `Files   11788 / 352252    ETA     06:14` (4-space gap, columns aligned). No test pinned width or column positions (existing assertions are substring/label-field only). 60 passed across the progress trio + lint. Mount protocol followed. NOT committed.

## 2026-06-14 (cont.) — Conflict dialog: live selection summary

Field ask on the overwrite/ConflictDialog (Matthew screenshotted a 76911-conflict copy): need a per-keypress indication of which method is selected so that at Enter-time the user knows what they've chosen. Design-first via AskUserQuestion — offered persistent-echo / timed-flash / live-summary; Matthew picked **live selection summary** ("unless you have better"). Delivered the summary with a small upgrade: it both confirms the keypress landed AND reports the true committed state.

New `_summary_text()` (pure, recomputed from `self._res` on every change) + `_refresh_summary()` (guarded on a `_summary_label` ref set in compose, so the existing pure-state tests that never mount stay no-op — same pattern as `_row_labels`/`_refresh_row`). Rendered as a bold `$warning`-coloured Label between the rows and the hint (`id="conflict-summary"`). Logic: uniform set → `Selected: all N -> OVERWRITE` (the common `S`/`O`/`R` case; single-row drops the "all N" → `Selected: SKIP`); mixed → per-method counts in display order (`Selected: 2 skip, 1 overwrite`). Wired `_refresh_summary()` into `action_set_current`, `action_set_all`, and the tail of `action_edit_name`. **Persistent, not fading** — chosen over a timed flash precisely because the info needs to survive until the user reaches for Enter.

6 new tests in `test_conflicts.py` (5 pure-state: default all-skip, set-all-collapse, mixed-breakdown, single-row wording, self-default mixed; 1 mounted pilot proving the on-screen label updates live on an `O` press — read via `str(lbl.render())`, the repo convention, after a `.renderable` AttributeError on Textual 8.x). **48 → 54 in that file**, lint clean. design.md: selection-summary paragraph in the Conflict-resolution-dialog section + decision-log rows for both this and the earlier ETA/box work. Mount protocol followed (sandbox stage, ast gate, md5 atomic push, `__pycache__` purge + touch). NOT committed — Matthew commits Windows-side.

## 2026-06-28 — Session re-orientation + 7-session roadmap

Picked WTree back up after a two-week gap. Reconciled memory against git: the 2026-06-14 batch (ProgressScreen box+ETA + ConflictDialog live selection summary) that my notes still flagged as UNCOMMITTED is in fact **HEAD `db26a25`** on `a02f1cb` (the field-fix batch, field run confirmed healthy). Only working-tree change is `Claude.md`, pure CRLF/LF line-ending churn — no substantive edits.

**Full suite verified green at HEAD.** Linux sandbox (Textual 8.2.7, pip-installed), run in 5 foreground file-chunks because the sandbox reaps long background processes after ~40s (that reaper + the crash-handler test printing its *expected* `RuntimeError: debug-probe` panel was the whole "stall at 16%" red herring — not a code hang). Chunk totals: 143 + 145(+1 skip) + 235 + 210 + 134 = **866 passed + 1 skipped** (867 collected), zero failures/errors across all 67 files. Used `--timeout=10 --timeout-method=signal` so any genuine hang would surface as a failure rather than block.

**Roadmap:** Matthew asked to batch the whole open backlog (~60 `- [ ]` items) into the next ~7 sessions. Drafted a dependency-ordered plan and spliced it into `todo.md` under a new top section "## Roadmap — next ~7 sessions (planned 2026-06-28)". Sequence: (1) Move/Delete executor hardening — cross-fs DIR move read-only exposure (the parked twin of the delete WinError 5 fix); (2) op-log follow-ups + in-app last-op viewer; (3) boundary-layer path normalisation; (4) tree-pane action parity (rename-from-tree + Backspace-ascend); (5) viewer overhaul pt.1 (body-widget swap + syntax highlighting + gutter); (6) viewer overhaul pt.2 (huge-file paging + hex + config ceiling); (7) daily-driver papercuts. Plus an Overflow bucket for medium items and the genuinely-parked parking-lot (cross-source translation, multi-source tree, soft-delete, batch rename, themes, sub-menus). Ordering rationale: normalisation (3) before tree-pane rename (4) since both touch tree-pane path handling; viewer body-swap (5) before paging/hex (6). No code changed this session — planning only.

## 2026-06-28 (cont.) — Session 1: cross-fs DIR move read-only hardening

First roadmap session. Closed the WinError 5 gap parked since the 2026-06-11 Delete read-only fix: `_native_move`'s cross-fs DIR branch delegated source cleanup to `shutil.move`, whose internal `rmtree` has no read-only retry — a read-only entry anywhere in the moved tree (git `.git/objects/` pack/object files) aborts the cleanup mid-walk with WinError 5 on Windows, leaving the source half-removed while the destination copy is already complete. POSIX puts deletion rights on the parent dir, so the Linux suite was structurally blind (same blindness class as Delete).

**Fix** (`wtree/ops/execute.py`): the DIR branch now does an `_is_within` `_destinsrc` guard (refuse copying a dir into its own subtree — copytree would recurse forever) → `shutil.copytree(src, dst, symlinks=True)` (mirrors `shutil.move`: links inside the tree preserved, not dereferenced) → `_rmtree_force(src)` (the read-only-tolerant remover already built for Delete). copytree and the rmtree are split into two `to_thread` calls so a post-copy cleanup failure is attributed distinctly as `remove source after copy` (the dir is then in both places — same partial-failure shape as the FILE branch's `unlink source after copy`). New pure helper `_is_within(child, parent)` via `canonical_path` (separator + case fold), immune to the `/a-x`-between-`/a`-and-`/a/b` lexicographic trap.

**Tests:** new `tests/test_move_readonly.py` (12) — a sanity pin that bare `shutil.move` refuses the staged read-only repo (proving the fixture stages the bug), the e2e survives-readonly-subdir (with and without a bytes callback), remove-source-after-copy attribution (monkeypatched `_rmtree_force`), into-itself refusal, inner-symlink preservation (skipif nt), and 6 `_is_within` units. Mirrors `test_delete_readonly.py`'s POSIX write-protected-dir staging (chmod 0o500 objects dir; euid-0 skip; finally force-writable so a red test never strands an undeletable tmp_path). Updated the now-stale "keeps shutil.move" comments in `test_move_chunk_hook.py` (renamed `test_cross_fs_dir_keeps_shutil_move` → `test_cross_fs_dir_move_succeeds`; assertions unchanged — observable behaviour is identical).

**Verified** (sandbox, mount protocol throughout — ast gate, atomic `os.replace`, `__pycache__` purge + touch): test_move_readonly + test_move_chunk_hook + test_lint = 23 passed + 1 skipped; broader move/ops_move/ops_execute/delete_readonly/self_target/conflicts/windows_paths = 138 passed. euid 1220 in-sandbox, so the read-only chmod-retry path genuinely executed (not root-skipped). Zero failures. docs updated (design.md DIR bullet + decision row; todo.md unpark + DONE item + roadmap Session 1 marker). NOT committed — Matthew commits Windows-side; the stale `.git/index.lock` still needs clearing first.

## 2026-06-28 (cont.) — Progress dialog goes file+byte hybrid (field-driven)

Matthew field-tested the progress dialog on a real 269,191-file copy of `-=Programming=-` and screenshotted the ETA misbehaving: it read **4:29:45**, then on the next shot **32:34:24**. Diagnosis: between the two, the copy plowed into the spirograph SVG forest — ~59,000 files for only ~27 MB (≈450 B each), so byte-throughput cratered to ~50 KB/s while the file-rate held (~113 files/s). The ETA (and the percent bar) were **purely byte-weighted**, so projecting the remaining ~20 GB at the depressed byte-rate exploded to 32h, and the bar read **0%** while 25% of the files were already done. The byte-only model is the wrong model for a file-count-bound copy. Matthew picked "build the hybrid (recommended)" via AskUserQuestion.

**Fix** (`wtree/widgets/progress_screen.py`): a cost model `elapsed ≈ a·byte_fraction + c·file_fraction`, fractions in [0,1] (normalising by the known totals keeps the normal matrix well-conditioned — raw byte counts squared would swamp the file counts), fit by least squares over a rolling window of throttled `(elapsed, byte_frac, file_frac)` samples (`_record_sample`, ≥1s apart, deque maxlen 600 ≈ 10 min). `a` ≈ byte-throughput cost, `c` ≈ per-file overhead. The **bar/percent** show the cost-weighted blend `(a·bf + c·ff)/(a+c)` when the fit is available, else `max(byte%, file%)` so it never sits at 0% mid-copy, held to a monotonic high-water mark so a model revision can't rewind it. The **ETA** projects `a·(1-bf) + c·(1-ff)` when fit, else the byte-only `_eta_seconds`, else a file-only `_eta_seconds(items)` (covers the all-tiny / empty-file regimes). **Drag stays byte-only by design.** `_fit_cost_model` returns None (→ byte fallback) on <4 samples, a singular/collinear matrix (uniform file sizes → byte_frac == file_frac → costs inseparable), or a negative coefficient (overfit/noise).

New pure helpers `_fit_cost_model` / `_model_eta` / `_model_fraction` (tested directly, the repo convention). 11 new tests in `test_progress_dialog.py` (recover-known-coeffs from a well-conditioned big-file→tiny-file regime curve; None on too-few/collinear/negative; eta pricing + clamping; fraction blend + zero-denominator fallback; body uses model-eta with injected samples; body file-eta fallback when bytes say nothing; hybrid percent shows the higher of byte/file with no model). Replaced the old `test_percent_is_byte_weighted_not_item_weighted` with a hybrid-aware version. Verified green: progress trio + minimize + safe-dismiss + lint (69 passed + 1 skip) and ops_queue + app + copy_e2e (22 passed). Mount protocol throughout (ast gate, atomic `os.replace`, `__pycache__` purge + touch).

**Parked refinement:** the rolling samples live on the screen instance, so `Ctrl+P` minimize/resume re-warms the fit from scratch (re-stabilises in ~10-20s). Moving the samples onto the queue (single source of truth) would survive minimize — worth it only if it ever bugs in the field. NOT committed — Matthew commits Windows-side (clear `.git/index.lock` first). Slots into the roadmap as a field-driven insert between Sessions 1 and 2.

## 2026-06-28 (cont.) — Session 2: op-log follow-ups (Ctrl+O last-op viewer + verbose env)

Second roadmap session. Closed both op-log follow-ups parked when the log shipped 2026-06-11. Design via AskUserQuestion (all three = my recommendations): Ctrl+O + Commands menu; viewer shows non-success by default with `a` to reveal all; `WTREE_OPLOG_VERBOSE` env var for verbose (the `WTREE_DEBUG` precedent — no config-persistence layer needed).

**(a) Last-operation viewer.** New `wtree/widgets/last_op.py`: pure `render_last_op(result, *, show_all=False)` (header = `summary()`; then non-SUCCESS items, or all when `show_all`; per-status colour; all-succeeded message + "press a" in the default view; "(N succeeded hidden)" note; empty-plan line; Delete arrow collapse) and `OperationResultScreen(ModalScreen[None])` (PropertiesScreen-shaped: VerticalScroll + header/body/hint, Esc/Q close, `a` toggles). App wiring: `action_show_last_op` (Ctrl+O) flashes "No completed operation yet" when `last_result is None` (Ctrl+P idle idiom), no-ops if a viewer is already stacked, else pushes the screen over the in-memory `last_result` — zero disk I/O, never touches the flaky mount or the log file. Commands-menu "Last operation" item (accel `o`, after Progress dialog) + Help Application row.

**(b) Verbose op-log.** `oplog.format_result` gained `verbose: bool | None` (None → reads `verbose_enabled()` which checks `WTREE_OPLOG_VERBOSE` against `{1,true,yes,on}`); when on, SUCCESS items get detail lines too. Extracted `item_arrow(item)` and `_item_line(r)` (shared with the viewer); the non-success format is byte-identical to before (incl. the `(no message)` placeholder — a SUCCESS with no message drops the suffix instead, so verbose logs stay clean). `write_result` passes the env-derived verbose through. MAX_DETAIL_LINES cap applies to whatever set is emitted.

**Tests:** new `tests/test_last_op.py` (8: render default/show-all/all-succeeded/empty/delete-arrow + pilot flash-when-no-result / open-toggle-close / no-double-stack) and 5 oplog verbose tests (env reader, verbose includes success, success-no-placeholder, default-still-omits, write-via-env). Updated `test_menu.py` (Commands list 8→9 with "Last operation"; the Untag-all down-walk 7→8). Verified green: last_op + oplog + menu + help + lint (53 passed + 1 skip) and menu_polish + status_keybar + app (34 passed). Mount protocol throughout (ast gate, atomic `os.replace`, `__pycache__` purge + touch). NOT committed — Matthew commits Windows-side (clear `.git/index.lock` first).

## 2026-06-30 — Session 3: boundary-layer path normalisation (born POSIX, displayed native)

Third roadmap session, the meatiest of the small ones (cross-cutting, Windows-only payoff). Closed the boundary-layer follow-up parked back in the 2026-06-11 backslash fix. Design via AskUserQuestion: **full born-POSIX scope** + **flip to native for display**.

**Root cause / concrete bug.** `Entry` carries only a basename, so full paths were built by consumers — both panes used native `os.path.join` (backslash on Windows), while planner destinations and typed paths were `posixpath`-joined (forward slash). Mixed separators. `TreePane.refresh_paths` matches `node.data in touched_paths` by **exact string**, and `touched_paths` used native `os.path.dirname`; so on Windows the Copy/Move *destination* node (backslash `data`) never matched its forward-slash dst key and the destination folder didn't auto-refresh after an op. Same root cause showed mixed separators in dialogs.

**Phase A — internal born-POSIX.** New `to_native(path, *, sep=os.sep)` in `ops/base.py` (sibling of `to_posix`; `sep` param so Windows display is POSIX-testable). Normalised every path entry point to `/`: `contents_pane` join + `show_path` entry; `tree_pane` root abspath, child `posixpath.join`, `reveal_path` target, `re_root` abspath, the expanded-paths sort (`p.count("/")`); `app._root_path` at all four assignment sites (ctor / ascend dirname / L-prompt resolve / switch-drive pick) + the find-tree reveal walk; `OperationResult.touched_paths` → `posixpath.dirname`. Re-exported `to_native` from `wtree.ops` (app imports from the package — that was the one collection error, caught + fixed immediately). OS calls unchanged: `execute._normalise_dst` already flips dst to native, and Windows accepts `/` for reads anyway.

**Phase B — native display flip.** `to_native` applied only at user-facing full-path renders: tree root label (+ `re_root`), Properties headers + Path rows (NOT the `os.lstat` call), progress current-item (`-> dst` / delete src), conflict-dialog rows, and the Copy/Move + Delete dialog headers + confirm bodies. The destination picker and `drive_anchor` were left fully native on purpose — the picker already displays native and round-trips its result through the existing boundary `to_posix`, so touching them would only have re-introduced posix display there.

**Verification.** On POSIX every change is a no-op (`to_posix`/`to_native` identity; `posixpath` == `os.path`), so the existing suite proves no behavioural regression, and 7 new POSIX-runnable string pins in `test_windows_paths.py` (helpers with explicit `sep`; round-trip; `touched_paths` stays POSIX for COPY single-parent + MOVE both-parents) cover the Windows-specific intent. Ran the whole path/widget/e2e surface in chunks — all green (tree/contents/nav/ascend/cursor/refresh/find + copy/move/delete/rename/make-new/view/edit e2e + properties/progress/conflicts + windows/picker/drive-switching/log-source + executor/status/menu/oplog/last-op): ~600+ executions, zero failures. Mount protocol throughout. NOT committed — Matthew commits Windows-side (clear `.git/index.lock` first).

**Field check wanted (Windows-side):** the real payoff — after a Copy/Move, the destination folder's tree node should now auto-refresh — can only be confirmed on Windows. Also worth an eyeball that the native display flip reads right everywhere (tree root label, the Copy/Move/Delete dialogs, Properties). Two Ascend-era spot-checks from the design also remain Windows-only: UNC-path ascend, and the symlink-at-root realpath-vs-literal call.

## 2026-06-30 (cont.) — Session 4: tree-pane action parity

Fourth roadmap session, de-risked by Session 3 (tree-pane paths are now clean POSIX). Closed the rename-from-tree follow-up parked when the 2026-06-11 Selection rule landed (the ops resolver was aligned to the focused pane then, but `action_rename` kept reading the contents pane directly), plus the parked Backspace-ascend gesture and two Navigation-era no-op-feedback items.

**Rename from tree.** New `WTreeApp._resolve_rename_target() -> tuple[str, Kind] | None`, mirroring `_resolve_selection_tags`: tree-focused → the cursor's directory node (`Kind.DIR`); contents-focused → the cursor entry as before. The logged **root** node is refused with a nudge — renaming the directory the tree is rooted at would orphan the root path. Decision via AskUserQuestion: disallow+nudge (over allow+re-root, which adds re-root/cursor/tag fixups, and fall-back-to-contents, which mixes panes). `action_rename` now calls the resolver instead of reading `ContentsPane.cursor_entry()` directly; everything downstream (tag → prompt → plan_rename → conflicts → finalise) is unchanged, and the tree auto-refreshes via the existing `touched_paths` (parent-of-src) path.

**Backspace disambiguation.** `TreePane.action_focus_parent` (Backspace) now: cursor on root → posts `AscendRequested` (re-root at parent, exactly like Left-on-root) instead of no-opping; any other node → cursor-to-parent (unchanged). The contents pane's Backspace delegates to the same method, so it ascends at the top too — Left-on-root and Backspace-on-root now agree. Resolves the "revisit tree-pane tagging Backspace" note too (Space tags, Backspace navigates — unambiguous).

**No-op nudges.** `ContentsPane.action_enter_dir` flashes a status nudge on a file/symlink row ("Right-arrow opens directories - press V to view a file.") and when `focus_dir_under_cursor` returns False ("Couldn't drill in - directory isn't in the tree."), via a guarded `_nudge` helper (`getattr(self.app, "flash", None)` so ContentsPane stays usable under a bare test host).

**Tests / verify.** New `tests/test_tree_rename.py` (8): rename-from-tree targets the dir node not its first child; root refused + nudged; contents-focused unchanged; resolver units (tree dir / root→None); Backspace-on-root ascends; Backspace-on-non-root cursor-to-parent; right-on-file nudge. Affected slices green (selection_focus / rename_e2e / ops_rename / rename_smart_cursor / tree_arrows / navigation / ascend / drillin / tree_refresh / cursor_preserve / status_keybar / app / self_target). The lone failure in one mixed run was `test_flash_clears_after_timeout` — the documented timer flake (12/12 green in isolation), unrelated to this change. Mount protocol throughout. NOT committed — Matthew commits Windows-side.

## 2026-06-30 (cont.) — Session 5: viewer overhaul pt.1 (syntax highlighting + gutter + BOM)

The big one, part 1. Design via AskUserQuestion: **Rich `Syntax`-derived Text** (keep the Static + VerticalScroll + the 2026-06-10 `/`-search machinery) over a Textual `TextArea` swap (which would have meant rebuilding the search); and **auto-cap + manual `h` toggle** for big-file highlighting. Key enabler: **Pygments already ships with Rich**, so highlighting adds no dependency.

**BOM detection.** `_load_file_sync` now checks for a byte-order mark *before* the NUL binary heuristic — a UTF-16 file is mostly NULs for ASCII and would otherwise be refused as binary. `utf-8-sig` strips a UTF-8 BOM; `utf-16` LE/BE decodes (the codec reads the BOM to pick endianness). UTF-8 → latin-1 fallback for the no-BOM case is unchanged. Also guesses a lexer per file via `Syntax.guess_lexer` (extension-based; `.py`→python, `.txt`→text, unknown→default), stored on `_LoadResult.lexer`.

**Highlighting + gutter.** Pure helpers (testable without the screen): `highlight_lines(text, lexer, *, enabled)` → per-line Rich Texts (whole-doc `Syntax(...).highlight()` then split), `line_start_offsets`, `gutter_width`, `render_body` (assembles `<gutter> <line>` per row + overlays the search-match spans using **line-relative columns**, so the gutter width never enters the offset maths). The screen caches `_disp_lines` (recomputed only on load / toggle — the costly Pygments pass) and rebuilds the cheap match overlay per keystroke. Gutter = right-aligned dim line numbers + `│`. Lexer/encoding/highlight-state shown in the header.

**Perf bug caught + fixed.** First cut sliced the highlighted Text per line (`highlighted[pos:pos+len]`) — Rich Text slicing is O(spans), so this was O(lines×spans) and *hung* (>45s) on a 2 MB file. Switched to Rich's O(lines+spans) `Text.split`, reconciling the trailing-empty-line difference vs `str.split` by padding + a per-line plain-equality guard. After the fix, highlighting is ~linear (sandbox, CPU-throttled: 95 KB 0.5s / 388 KB 1.9s / 1 MB 4.9s — sub-second on real hardware). Set `HIGHLIGHT_MAX_BYTES = 512 KB` as the default-on cap; `h` forces it either way.

**Tests.** 18 new (5 BOM/lexer load units, 5 highlight/gutter/render_body pure units, 2 pilot — `h` toggle + big-file auto-cap-off via a monkeypatched tiny cap). The existing `/`-search span assertions were made **gutter-agnostic**: `_spans` now filters to the yellow/cyan match spans (the gutter adds `dim` spans), and the one absolute-offset assertion now checks the underlying display text equals the query. test_viewer + test_viewer_search + test_view_e2e all green (50 passed + 1 skip). Mount protocol throughout. NOT committed — Matthew commits Windows-side.

**Session 6 (next) sets up cleanly:** hex mode and huge-file paging are just different body Texts feeding the same render path; the configurable `MAX_BYTES` seeds the first runtime-config surface.

## 2026-06-30 (cont.) — ConflictDialog memory fix + queue retention cap (field-driven)

Matthew field-tested another huge re-copy of `-=Programming=-` (356,239 conflicts, everything already at the destination) and screenshotted the Python process at **~7 GB / 96% memory**. He wondered if it was just the GC. Diagnosis: **`ConflictDialog.compose` mounted one Textual `Label` widget per conflict** — 356k widgets — and `_restyle_rows`/`_refresh_all_rows` iterated all N on every keystroke. Textual widgets are heavy (reactive state, region, styles, DOM node), so that's the ~7 GB, and they're *live* (reachable) while the dialog is open, so GC can't touch them. (His GC hunch was right for the *reclamation* side — Textual widgets form parent↔child cycles, so after the dialog closes they're freed only by the cyclic collector, which can lag; but the root cause is allocating 356k widgets, which no GC helps while it's open.) He picked the fix via AskUserQuestion: windowed single Static; cap `_completed` only.

**Conflict dialog → windowed single Static.** Replaced the N `Label`s with one `Static` body showing a viewport of `_VISIBLE_ROWS = 18` rows around the cursor. New `_window_text()` (renders only the visible slice, cursor row styled `reverse`; O(visible)), `_ensure_cursor_visible()` (slides `_top` so the cursor stays in frame), `_refresh_body()` (updates the body + a new `row X of N` position line). Navigation rewritten onto a `_move_cursor(target)` helper + added PgUp/PgDn/Home/End for big lists. `set_all` is now a one-line `_res = [res]*N` list-fill (instant at 356k) and only re-renders the window. Crucially `_row_text` / `_summary_text` / the resolve + `(resolutions, custom_dsts)` commit contract are **unchanged**, so all the existing conflict tests (which read `_row_text(i)` and the live `#conflict-summary` label) still hold. Removed `_row_labels`/`_refresh_row`/`_refresh_all_rows`/`_restyle_rows`/`_scroll_to_cursor` and the now-unused `VerticalScroll` import.

**Queue retention cap.** `OperationQueue._completed` was an unbounded `list[OperationResult]` (each result holds one `ItemResult` per item) — a slow leak across a session of big ops. Added `MAX_COMPLETED = 50` and trim-on-append (`del self._completed[:-MAX_COMPLETED]`). Tests that read `completed[0]`/`completed[-1]`/`len(completed) == 2` still pass (they run ≤ 2 ops).

**Verify.** 6 new tests: `_window_text` caps at `_VISIBLE_ROWS`; window follows the cursor; `_top` clamps to 0; `row X of N`; a **50k-conflict mount that asserts ≤ 6 Labels** (the memory-regression pin — was 50k); queue `_completed` capped (monkeypatched tiny cap, 6 plans → 3 retained). conflict + ops_queue + lint green (66), and copy/move/self-target/midplan/plan-cancellable/progress/app green (104). Smoke: a 100k-item dialog constructs in 0.18s and `set_all` list-fills in 0.3 ms. Mount protocol throughout. NOT committed.

**Parked follow-up (noted in todo):** slim `last_result` to summary + non-success items (drops the SUCCESS bulk; also makes the Ctrl+O viewer scale) and release/slim `last_plan` (tests read `.items[0]`/`.kind`/`.file_count`, so it needs a careful touch) — a smaller, separate contributor than the dialog. Next up after this: Session 6 (viewer pt.2).

## 2026-06-30 — Session 6: viewer overhaul pt.2 (paging + hex + config + symlink)

Closed the four remaining View-era items. Decisions via AskUserQuestion: **head + `m` load-more** (over lazy-auto-on-scroll / just-raising-the-wall) for huge files, and an **env var** (`WTREE_VIEW_MAX_BYTES`, over a config file) for the configurable ceiling.

**Paging (retire the hard refusal).** `_load_file_sync(path, *, limit)` no longer refuses a big file: it reads the first `limit` bytes, decodes, and sets `truncated = size > loaded`, carrying `loaded_bytes` / `byte_size` (total) / `data` (raw bytes, for hex). The screen's `m` key (`action_load_more`) grows `self._limit` by a page and re-loads, restoring scroll so the new content appears below. Re-decoding the whole loaded prefix each time is O(loaded) but dead simple and sidesteps UTF-8 page-boundary splits; a split multi-byte char at the very end of a truncated read is trimmed (`e.start >= len(data) - 3`) so the encoding stays labelled utf-8 rather than mislabelled latin-1. `MAX_BYTES` is now the per-page default (4 MiB), env-overridable via `_max_bytes()` (bad / non-positive → default; config must never make a file unviewable).

**Hex mode.** Binary files (NUL bytes, no text BOM) are no longer refused — they load their bytes and default to the hex view (`_render_mode = "hex" if is_binary`). `x` (`action_toggle_hex`) flips text↔hex on any file. Pure `hex_lines(data, *, base)` renders `OFFSET  HH HH ..  |ascii|`, 16 bytes/row (non-printable → `.`); `render_body` gained a `gutter=` flag so hex skips the line-number gutter (the byte offset is its own gutter). `_prepare_body` / `_apply_highlights` branch on the mode; search is text-mode-only (`_searchable` now also requires text mode).

**Symlink message.** `_stat_refusal(path, exc)` gives a dangling link `symlink target missing: <target>` (ENOENT) or `symlink loop: ...` (ELOOP) instead of the bare `could not stat`.

**Tests.** 12 new / rewritten: env override (valid / nonsense / 0 / unset), `hex_lines` format + base offset + multi-row, oversize-now-truncates + full-under-limit, binary-not-refused-+-flagged, binary-defaults-to-hex pilot, load-more pages-in pilot (40→80→100), x-toggle pilot, dangling-symlink message (skipif nt). The old hard-refusal / binary-refusal tests were rewritten to the new behaviour. Viewer + search + e2e + lint green (57 + 1 skip), app + edit green (30). Mount protocol throughout. NOT committed.

**Roadmap status:** all 7 sessions of the 2026-06-28 roadmap are now done (1 cross-fs move; 2 op-log; 3 boundary paths; 4 tree-pane parity; 5 viewer pt.1; 6 viewer pt.2; 7 daily-driver papercuts is the only one left) — plus the two field-driven inserts (progress hybrid; conflict-dialog memory). Session 7 (papercuts) is the remaining roadmap item.

## 2026-06-30 — Session 7 (input-safety group): the last roadmap item

Matthew picked the **input-safety** group of the loose Session 7 papercut batch (left the make-new / search-polish / infra groups for later). Three fixes.

**Validation-on-Enter for Copy/Move.** New `WTreeApp._destination_error(dest)` (static) pre-flights a typed destination: it walks up to the nearest EXISTING ancestor (the executor makedirs missing leaf dirs, so a not-yet-existing destination is fine as long as its nearest existing ancestor is a writable directory - not over-strict) and returns a clear message for unreachable / not-a-directory / not-writable / dest-is-already-a-file. `_plan_modal_enqueue` calls it right after the typed destination is `to_posix`'d and flashes the error instead of letting the op fail mid-flight.

**Windows reserved-names check.** `is_reserved_name(name, *, windows=os.name=='nt')` in `ops/base.py` - CON/PRN/AUX/NUL/COM1-9/LPT1-9, case-insensitive and extension-agnostic (`CON.txt` is reserved). The `windows=` flag (default = platform) makes it a no-op on POSIX (so a genuine `CON` file on Linux stays valid) while being unit-testable on a POSIX host. Wired into `plan_rename` (after the source-exists + separator checks) and `resolve_relative_leaf` (Make-new + the conflict dialog's custom-rename editor), both rejecting with `InvalidName`.

**Editor opt-out.** `resolve_editor` now distinguishes "unset" from "set-but-empty": it walks VISUAL then EDITOR, and the first var *present in the environment* decides - non-empty → use it, empty → raise the new `EditorDisabled` (honours a deliberate `EDITOR=""` rather than dropping the user into the platform default). `action_edit` catches it and flashes the reason.

**Tests.** 9 new + 1 rewritten: `is_reserved_name` units (windows true/false) + `resolve_relative_leaf` reserved-rejection + `plan_rename` reserved-rejection (both forcing windows via a captured-original monkeypatch to avoid recursion); `_destination_error` (existing dir ok / new-subdir ok / file rejected / read-only parent rejected, euid-0-skipped); editor empty-VISUAL-disabled + empty-EDITOR-disabled (the old `test_empty_visual_treated_as_unset` was rewritten to the new contract). Affected slices green: path_norm/rename/copy_e2e/edit/lint (72) + make_new/conflicts/self_target/move/app/windows_paths (177). Mount protocol throughout. NOT committed.

**ROADMAP COMPLETE.** All 7 sessions of the 2026-06-28 roadmap are done (1 cross-fs move read-only; 2 op-log follow-ups; 3 boundary path normalisation; 4 tree-pane parity; 5 viewer pt.1; 6 viewer pt.2; 7 input-safety papercuts), plus the two field-driven inserts (progress file+byte hybrid; conflict-dialog memory fix). Remaining Session 7 sub-groups (make-new ergonomics, search polish, infra nits) are parked as optional future polish in todo.md.
